  ...
  Private Sub frmMain_SizeChanged(sender As Object, e As System.EventArgs) Handles Me.SizeChanged
    ...
    Else
      ...
      Else
        ...
        If fRatio = 0.0! Or Single.IsInfinity(fRatio) Or Single.IsNaN(fRatio) Then fRatio = Me.Font.SizeInPoints / 200
  ...
  Private Sub DisplayRResults(lDown As Long, lDownLim As Long, lUp As Long, lUpLim As Long, sLastUpdate As String)
    ...
    If mySettings.Overuse > 0 Then
      ...
      If TimeCheck <= -15 Then
        Dim lItems() As DataBase.DataRow = Array.FindAll(usageDB.ToArray(mySettings.HistoryInversion), Function(satRow As DataBase.DataRow) satRow.DATETIME.CompareTo(Now.AddMinutes(TimeCheck)) >= 0 And satRow.DATETIME.CompareTo(Now) <= 0)
